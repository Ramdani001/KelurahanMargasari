<?php
    ...
    function getKode($conn){
        ...
        $query = mysqli_query($conn, "SELECT * FROM tblpindah ORDER BY id DESC LIMIT 1");
        $row = mysqli_fetch_assoc($query);
        if($row == null){
            $noPelayananGenerate = "1 PDH/".date("Y");
        }else{
            $noPelayananGenerate = ($row["id"]+1)." PDH/".date("Y");
        }
    ...
    function tambahData($conn) {
        ...
        $query = "INSERT INTO tblpindah
        (noPelayanan, nik, tanggalPengajuan, jenisPelayanan, keterangan, `status`, createdBy, createdAt)
        VALUES ('$noPelayanan', '$nik', '$tanggalPengajuan', '$jenisPelayanan', '$keterangan', '$status', '$createdBy', '$createdAt')";
    ...
    function readDataTable($conn){
        ...
        $querycount = mysqli_query($conn, "SELECT count(id) as jumlah FROM tblpindah ".$where);
        ...
        if(empty($_POST['search']['value'])) {            
            $query = mysqli_query($conn, "SELECT * FROM tblpindah $where order by $order $dir LIMIT $limit OFFSET $start");
        } else {
            $search = $_POST['search']['value']; 
            
            $query = mysqli_query($conn, "SELECT * FROM tblpindah 
            ...
            $querycount = mysqli_query($conn, "SELECT count(id) as jumlah FROM tblpindah 
                WHERE noPelayanan LIKE '%$search%' 
                or nik LIKE '%$search%' 
                or tanggalPengajuan LIKE '%$search%' 
                or jenisPelayanan LIKE '%$search%' 
    ...
    function hapusData($conn) {
        $id = $_POST["id"];
        $query = "DELETE FROM tblpindah WHERE id = $id";
        mysqli_query($conn, $query);
        echo 200;
    }
    ...
    function gantiStatus($conn) {
        $id = $_POST["id"];
        $status = $_POST["status"];
        $query = "UPDATE tblpindah SET `status` = '$status' WHERE id = '$id'";
        mysqli_query($conn, $query);
        echo 200;
