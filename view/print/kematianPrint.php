<style>
    #judul{
        text-align:center;
    }
    
    #halaman{
        width: auto; 
        height: auto; 
        position: absolute; 
        border: 1px solid; 
        padding-top: 30px; 
        padding-left: 30px; 
        padding-right: 30px; 
        padding-bottom: 80px;
    }
</style>
    

<div id="libInp" style="display:none">
    <div id=halaman>
        <h3 id=judul>SURAT PEMBERITAHUAN KEMATIAN</h3>

        <p>Saya yang bertanda tangan di bawah ini :</p>

        <table>
            <tr>
                <td style="width: 30%;">Nama</td>
                <td style="width: 5%;">:</td>
                <td style="width: 65%;"  id="namaPrint"></td>
            </tr>
            <tr>
                <td style="width: 30%;">NIK</td>
                <td style="width: 5%;">:</td>
                <td style="width: 65%;" id="nikPrint"></td>
            </tr>
        </table>

        <p>Menyatakan pemberitahuan kematian Dengan Nomor Pelayanan <b id="noPelayananPrint"></b> dengan Keterangan <div id="keteranganPrint"></div> </p>

        <div style="width: 50%; text-align: left; float: right;" id="tanggalPrint"> </div><br>
        <div style="width: 50%; text-align: left; float: right;">Yang bertanda tangan,</div><br><br><br><br><br>
        <div style="width: 50%; text-align: left; float: right;" id="tandatanganPrint"></div>
    </div>
</div>